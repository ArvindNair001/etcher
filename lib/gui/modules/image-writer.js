...
imageWriter.service('ImageWriterService', function ($q, $rootScope) {
   ...
   *
   * @example
   * const entryPoint = ImageWriterService.getApplicationEntryPoint()
   */
   ...
   *
   * @example
   * ImageWriterService.performWrite('path/to/image.img', {
   *   device: '/dev/disk2'
   * }, (state) => {
  ...
  this.performWrite = (image, drive, onProgress) => {
    ...
    return $q((resolve, reject) => {
      ...
      const argv = _.attempt(() => {
        const entryPoint = this.getApplicationEntryPoint()
